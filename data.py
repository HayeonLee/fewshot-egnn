...
class TieredImagenetLoader(data.Dataset):
    def __init__(self, root, partition='train'):
        ...
        tt.utils.tic()
        #path = osp.join(root, 'tiered-imagenet', 'imtiered_debug' + '.pt')
        #print(path)
        #data = pickle.load(open(path, 'rb'))
        if partition == 'train':
            path = osp.join(root,'tiered-imagenet',  'imtiered_' + partition + '-1.pt')
            data = pickle.load(open(path, 'rb'))
            print('load imtiered_train-1.pt')
            path = osp.join(root, 'tiered-imagenet', 'imtiered_' +  partition + '-2.pt')
            data += pickle.load(open(path, 'rb'))
            print('load imtiered_train-2.pt')
        else:
            path = osp.join(root, 'tiered-imagenet', 'imtiered_' +  partition + '.pt')
            print(path)
            data = pickle.load(open(path, 'rb'))
        dt = tt.utils.toc()
        ...
        self.data = data
        self.full_class_list = torch.arange(len(self.data)).to(self.device)

        print('=> Load Tiered ImageNet [{:.3f}min] {} data size: {}'.format(dt/60, partition, len(data)))

    def random_sample(self, data, num):
        rand = torch.randperm(len(data)).to(self.device)
        return data[rand[:num]]
    ...
    def get_task_batch(self,
        ...
        with torch.no_grad():
            ...
            # for each task
            for t_idx in range(num_tasks):
                task_support_data, task_query_data = [], []
                # define task by sampling classes (num_ways)
                task_class_list = self.random_sample(self.full_class_list, num_ways)
     
                # for each sampled class in task
                for c_idx in range(num_ways):
                    # sample data for support and query (num_shots + num_queries)
                    class_data_list = self.random_sample(self.data[task_class_list[c_idx]], 
                                                         num_shots + num_queries
                                                         ).to(self.device)
                    # load sample for support set
                    task_support_data.append(class_data_list[:num_shots])
                    task_query_data.append(class_data_list[num_shots:])

                task_support_data = torch.stack(task_support_data).view(-1, 3, 84, 84)
                task_query_data = torch.stack(task_query_data).view(-1, 3, 84, 84)

                support_data.append(task_support_data)
                query_data.append(task_query_data)

            support_data = torch.stack(support_data)
            query_data = torch.stack(query_data)

            label = torch.FloatTensor([i for i in range(num_ways)]).view(-1, 1).to(self.device)
            # support_label = label.expand(num_ways, num_shots).contiguous().view(1, -1).repeat(num_tasks, 1)
            support_label = label.repeat(1, num_shots).view(1, -1).repeat(num_tasks, 1)
            query_label = label.repeat(1, num_queries).view(1, -1).repeat(num_tasks, 1)
